...
def makeImage(tif_path, inter_window, mask_window, mask_size, mask_name,
              masks_folder, out_folder, out_format):
    with rasterio.open(tif_path) as tif:
        temp_arr = np.moveaxis(tif.read(window=inter_window), 0, -1)
    found_overlap = 0
    ...
    img_arr = np.zeros((int(mask_window.width), int(mask_window.height),
                        temp_arr.shape[2]))
...
def tifToTiles(tif_path, masks_folder, out_format, mask_folder_id, out_folder, **kwargs):
    ...
    if (len(intersecting) <= 0):
        print('No intersecting areas found.')
        return
    ...
    with rasterio.Env():
        #with rasterio.open(tif_path) as tif:
        batch_size = max(num_cores*8, int(len(inter_windows)/100))
        b_range = range(0, len(inter_windows), batch_size)
        for step in tqdm(b_range):
            saved_images = pool.starmap_async(makeImage,
                                        [(tif_path,
                                            ...
                                            inter_window in
                                            inter_windows[step:step+batch_size]])
            saved_images.get()
            del saved_images
...
parser.add_argument('-o', '--out_folder', dest='out_folder', default=None,
                    type=str,
                    help='Folder to create subfolder in to output ' +
                    'the saved_images.')
