...
def tileEverything(shps_folder, tiffs_folder, out_width, out_height,
    ...
    masks_folders = Parallel(n_jobs=num_cores)(delayed(tileShp)(shp.path, pixel_res)
                                               for shp in os.scandir(shps_folder))
...
parser.add_argument('-s', '--shps_folder', dest='shps_folder', type=str,
                    required=True, help='Path of the folder containing the masks.')
